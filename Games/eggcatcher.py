...
c.create_rectangle(-5,canvas_height-50,canvas_width+5,canvas_height+5,fill='green',width=0)
c.create_oval(-80,-80,120,120,fill='orange',width=0)
c.pack()

color_cycle=cycle(['light blue','pink','purple','red','yellow','black','white','light green'])
egg_width=45
egg_height=55
egg_score=10
egg_speed=500
...
def egg_dropped(egg):
    ...
    c.delete(egg)
    if lives_remaning==0:
        messagebox.showinfo('GAME OVER!!!YOU ARE A LOSER!!','Final Score:'+ str(score))
        win.destroy()
...
def catch_check():
    ...
    for egg in eggs:
        (egg_x,egg_y,egg_x2,egg_y2)=c.coords(egg)
        if catcher_x<egg_x and catcher_x2>egg_x2 and catcher_y2-egg_y2<30:
            eggs.remove(egg)
            c.delete(egg)
...
def increase_score(points):
    ...
    score+=points
    egg_speed=int(egg_speed*difficulty_level)
    egg_interval=int(egg_interval*difficulty_level)
