...
def collision_with_attacker(attaker_pos,snake_pos):
    if attaker_pos in snake_pos[:]:
        return 1
    else:
        return 0

#direction blocked
def isdirectionblocked(snake_pos,current_dir_vector,attaker_pos):
    next_step=snake_pos[0]+current_dir_vector
    snake_head=snake_pos[0]
    if collision_with_boundaries(snake_head)==1 or collision_with_self(snake_pos)==1 or collision_with_attacker(attaker_pos,snake_pos)==1:
        return 1
    else:
        return 0
...
def play_game(snake_head,snake_pos,apple_pos,button_direction,apple_image,score,attaker_pos):
    ...
    while crashed is not True:
        ...
        display_attackers(attaker_pos[0],attaker_pos[1])
        ...
        if isdirectionblocked(snake_pos,current_dir_vector,attaker_pos):
            crashed=True
        clock.tick(10)
